...
countries_pop <- data.frame(country = countries, population = populations);
ordered <- countries_pop[order(countries_pop[, "population"]),]
ordered;
...
continents <- continents[c(4, 1, 5, 2, 3)];
continents;
continents <- c("America", "Asia", "Europe");
continents <- continents[c(3, 1, 3, 1, 2)];
continents;
countries_pop$continent <- continents;
countries_pop;
...
# of the animals in their shops which are older than four years old. Get this list as
...
official_list <- df[df$age >= 5, c("name", "type")];
official_list;
...
# Some error muts have ocurred when setting the age of Algernon, as its real age is the same
# Salem's age. Could you please correct this information in 'new_pets'

new_pets$age[2] <- new_pets$age[new_pets$name == "Salem"];
new_pets;
...
colors <- c("brown", "black", "grey", "white", "black", "grey");
df <- cbind(df, colors);
df;
...
# 'lost_dog' is contained in 'df' variable name. Suggestion: help(match).
