...
# (*) Exercise 1: Check if 'random_number' is divisible by 'random_dividend' using
# the modulo or remainder operator. Save the output in a logical variable called 
# 'divisible'. Read the documentation of floor() and runif() if you want to
# learn more about these functions.

random_number <- floor(runif(1, 11, 100));
random_dividend <- floor(runif(1, 1, 10));

divisible <- random_number %% random_dividend == 0;
...
eow <- strptime(EOW, format = "%S_%d %YA%M-%H/%m");
remaining_life <- as.numeric(difftime(eow, Sys.time(), units = "secs"));
...
vector <- seq(1, 8, length.out = 10);
vector;
vector <- vector[vector >= 2 & vector <= 5];
vector;
...
sort(random_v);
sort(random_v, decreasing = TRUE);
random_v[order(random_v)];
random_v[order(-random_v)];
...
gsub("a thousand", "ten millions", text);
...
c_matrix <- matrix(planets, nrow = 2, ncol = 4);
c_matrix[grepl("a", c_matrix)];
...
countries_pop[order(as.numeric(countries_pop[,"population"])),]
...
# Suggestion: Read the documentations of readRDS() and table(). You can use sort() on the
# output of table.

quixote <- readRDS("/home/falendor/Desktop/IE/sessions/2-3/quixote.RData");
sort(table(quixote));
sort(table(quixote), decreasing = TRUE)[1];
